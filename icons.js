// Iconos Lucide (https://lucide.dev, licencia ISC) compartidos por index.html y gobierno.html.
// Solo el contenido del <svg>; lucide(name) lo envuelve con los atributos estándar (1em, trazo 2, currentColor).
const LUCIDE = {
  ...
  // Usados por gobierno.js (Reasoning Replay: estado de la validación humana)
...
function lucide(name, cls = 'lucide-svg') {
  return `<svg class="${cls}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${LUCIDE[name] || ''}</svg>`;
}

// Sustituye los <i data-lucide="nombre"> del HTML estático por el SVG correspondiente.
function hydrateLucide(root = document) {
  root.querySelectorAll('[data-lucide]').forEach((el) => {
    el.outerHTML = lucide(el.dataset.lucide, `lucide-svg${el.className ? ` ${el.className}` : ''}`);
  });
}
// Hidratación automática del HTML estático en cuanto el DOM está listo (sin depender de app.js / gobierno.js)
if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => hydrateLucide());
else hydrateLucide();
